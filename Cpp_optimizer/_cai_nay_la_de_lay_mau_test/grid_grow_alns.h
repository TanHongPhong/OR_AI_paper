...
// =============================================================================
struct ALNSParams {
    int iterations = 500;  // Reduced for global destruction (more expensive)

    // --- destroy intensity (GLOBAL: very aggressive) ---
    // fraction of members to remove per trip (50%..85%)
    double destroy_frac_min = 0.50;
    double destroy_frac_max = 0.85;

    // connected-region removal target size factor
    double region_frac_min = 0.50;
    double region_frac_max = 0.85;

    // how many attempts of local improvement per iteration
    int vnd_tries = 40;
    ...
    // --- logging ---
    int log_interval = 50;   // print progress every N iterations
    ...
    // probability to wipe a random trip to enable reseed (was 0.30, now 0.40)
    double wipe_trip_prob = 0.40;      
...
void recompute_center(Trip& T) const {
    ...
    // --- GLOBAL Destruction: destroy ALL trips in one go ---
    void apply_destroy(State& S, const ALNSParams& P, int op) {
        // Collect all non-empty trips
        std::vector<int> nonempty_tids;
        nonempty_tids.reserve(S.trips.size());
        for (int i = 0; i < (int)S.trips.size(); ++i) {
            if (!S.trips[i].members.empty()) nonempty_tids.push_back(i);
        }
        if (nonempty_tids.empty()) return;
        
        // Shuffle to add randomness
        std::shuffle(nonempty_tids.begin(), nonempty_tids.end(), rng_);

        // Apply destruction to ALL trips (or a large fraction)
        for (int tid : nonempty_tids) {
            if (S.trips[tid].members.empty()) continue; // could have been wiped
            switch (op) {
