...
inline int spread_ring_limit(VehicleType t) {
    // Ring counts = Chebyshev distance in grid cells (10m per cell).
    // 2.5km = 250 cells for largest vehicles, decreasing for smaller ones.
    // Increased by +50 as requested
    switch (t) {
        case VehicleType::BIKE:        return 150;
        ...
        case VehicleType::UNKNOWN:     return 200;
    }
    return 200;
}
...
inline int fallback_seed_nearest_depot(
    const Container& unassigned_set,
    const std::string& depot_id,
    const Instance& inst)
{
    int best = -1;
    double best_d = 1e100;
    for (int cell : unassigned_set) {
...
// Check if cell touches members (8-neighbor) for frontier-only add
template <typename Container>
inline bool touches_members_8_init(
    const Instance& inst,
    int cell,
    const Container& members_set
) {
    int r = 0, c = 0;
    inst.customers.decode_linear(cell, r, c);
    for (int dr = -1; dr <= 1; ++dr) {
        for (int dc = -1; dc <= 1; ++dc) {
            ...
            int rr = r + dr, cc = c + dc;
            if (!inst.customers.in_bounds(rr, cc)) continue;
            int nb = inst.customers.linear(rr, cc);
...
public:
    ...
    static Solution create_initial_solution(Instance& inst, int seed = 42) {
            ...
            while (progressed && pass < MAX_PASSES) {
                ...
                for (const auto& vid : veh_list) {
                    ...
                    while (vehicle_active) {
                        ...
                        // This allows vehicles to reach far out without hard territory limits.
                        ...
                        for (const auto& sc : seed_cands) {
                            ...
                            const double dkm = sc.first / 1.0; // remove noise scale for actual check? No, use original
                            
                            // Re-get clean distance for actual constraint check
                            std::string rep_cid = rep_customer_id_from_cell(inst, cell);
                            double actual_dkm = inst.get_dist_km(did, rep_cid);
                            double trip_km = 2.0 * actual_dkm;
        ...
        std::cout << "[Init] Solution created. Handing over to ALNS.\n";
