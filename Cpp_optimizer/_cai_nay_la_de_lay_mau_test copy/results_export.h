...
// Helper to get customer ID from cell
...
inline void export_routes(const Instance& inst, const Solution& sol, const ExportConfig& cfg) {
    ...
    fout << "vehicle_id,trip_idx,depot_id,center_cell_linear,center_row,center_col,center_customer_id,trip_km,member_cells,load_w,load_v,load_w_pct,load_v_pct\n";
    ...
    for (const auto& kv : sol.routes) {
        ...
        for (size_t t = 0; t < vec.size(); ++t) {
            ...
            std::string center_cid = rep_customer_id_from_cell(inst, rt.center_cell_linear);
            ...
            // Add percentages
            double cap_w = 0.0, cap_v = 0.0;
            if (itV != inst.vehicles.end()) {
                cap_w = itV->second.cap_weight;
                cap_v = itV->second.cap_volume;
            }
            double pct_w = (cap_w > 1e-9) ? (rt.load_w / cap_w * 100.0) : 0.0;
            double pct_v = (cap_v > 1e-9) ? (rt.load_v / cap_v * 100.0) : 0.0;

            fout << "," << std::fixed << std::setprecision(1) << pct_w
