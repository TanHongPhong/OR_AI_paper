...
// ==========================================
//  Seed & Territory-Free Strategy Parameters
// ==========================================
struct SeedPolicyParams {
    int sample_n = 800;
    int max_relax = 3;
    ...
    int R_attach0 = 8;
    int R_attach1 = 20;
    int R_attach2 = 999999;
    double attach_bonus = 1.0;
    double attach_penalty = 0.2;
    ...
    // Near-first seeding threshold
    double high_pressure_nearfirst_th = 50.0;

    // --- Territory-Free Center Cohesion ---
    double w_adj = 15.0;              // weight for nearest-neighbor center penalty
    double R_adj = 0.6;               // max radius (km) for adjacency
    double w_cc = 200.0;              // weight for (num_cc - 1) penalty
    
    bool soft_kd_enabled = true;      // sample additional cells from high-pressure depots
    int seed_ring_max = 200;          // max ring search for seed near existing center
    int seed_sample_global = 400;     // fallback global sample size
    double w_patch = 0.5;             // bias towards existing center patch
};

struct SeedDebugInfo {
    std::uint64_t calls = 0;
    std::unordered_map<std::string, std::uint64_t> rescued_by_other;
    
    void on_rescued(const std::string& d1_depot) { rescued_by_other[d1_depot]++; }
    
    void print_summary(std::ostream& os, const std::string& tag) const {
        os << "\n[SeedDebug] " << tag << "\n";
        if (!rescued_by_other.empty()) {
            os << "rescued_d1_by_other: ";
            for (const auto& kv : rescued_by_other) os << kv.first << "=" << kv.second << " ";
            os << "\n";
        }
    }
    void reset() { calls = 0; rescued_by_other.clear(); }
};

struct DepotBalanceStats {
    std::unordered_map<string, int> served_by_depot;
    std::unordered_map<string, int> unserved_d1;
    std::unordered_map<string, double> pressure;
    
    void print(std::ostream& os) const {
        os << "\n[KPI-B] Depot Balance:\n";
        for (const auto& kv : served_by_depot) {
            int unserved = (unserved_d1.count(kv.first)) ? unserved_d1.at(kv.first) : 0;
            double p = (pressure.count(kv.first)) ? pressure.at(kv.first) : 0.0;
            os << "  " << kv.first << ": served=" << kv.second 
               << " unserved_d1=" << unserved << " pressure=" << p << "\n";
        }
    }
};
