...
struct State {
  ...
  bool finalize_cluster(int cid) {
    ...
    if (!recompute_center(C)) return false; // center/connectivity still hard?

    // Penalties for depot load / vehicle km
    double old_pen = 0.0; 
    // We didn't track old penalties of depot/veh-km in State explicitly to subtract.
    // This is tricky. 
    // Simplified: Assume hooks calculate delta? No, hooks receive NEW TOTAL.
    // We need to know OLD penalty to subtract.
    // Hack: Calculate Old Penalty using Old Value.
    // But we don't have old value (trip is updated).
    
    // For now, let's assume Depot Load and Vehicle KM checks are Hard-ish or handled by logic?
    // User only emphasized "Tràn chuyến" (Capacity).
    // Let's keep Depot Load / Vehicle KM check as Soft returning Penalty?
    // But implementation requires tracking running penalty.
    
    // Compromise: Keep Depot/KM as HARD for now (Constraint) or just Penalty without tracking?
    // If not tracking, total_cost drifts.
    // Let's track:
    // double prev_km_pen = hooks.vehicle_km_penalty(vid, vehicle_total_km[vid]);
    // vehicle_total_km[vid] += (C.trip_km - old_trip);
    // double new_km_pen = hooks.vehicle_km_penalty(vid, vehicle_total_km[vid]);
    // total_cost += (new_km_pen - prev_km_pen);

    double old_km_pen = hooks.vehicle_km_penalty(vid, vehicle_total_km[vid]);
    apply_trip_change(vid, old_trip, C.trip_km); // updates vehicle_total_km
    double new_km_pen = hooks.vehicle_km_penalty(vid, vehicle_total_km[vid]);
    
    total_cost += (new_km_pen - old_km_pen);

    // Depot load handled similarly? Depot load updated in add/remove.
    // We need to update penalty diff there.
...
struct GrowOps {
  ...
  static Cand pick_best_candidate(State& st, int cid, const GridGrowALNSConfig& cfg) {
    ...
    const double cy = C.sumy / (double)C.members.size();
    Pt centroid{cx, cy};
    Pt center_pt = (C.center_ui >= 0) ? st.customer(C.center_ui).p : centroid;
    
    // SEEDING: If cluster is empty, use Depot as seed
    if (C.members.empty()) {
        center_pt = st.depot(C.depot_id).p;
        centroid = center_pt;
    }
    ...
    cand.reserve(std::max((size_t)100, boundary.size() * 12));

    CellKey nb[8];
    
    if (C.members.empty()) {
        // Empty cluster: Seed from Depot location
        Pt dp = st.depot(C.depot_id).p;
        int r = (int)std::floor(dp.y / st.grid.cell_size_km);
        int c = (int)std::floor(dp.x / st.grid.cell_size_km);
        CellKey dc = pack_cell(r, c);
        
        auto add_cell = [&](CellKey k) {
            if (auto it = st.grid.cell2cust.find(k); it != st.grid.cell2cust.end())
                for (int u : it->second) cand.insert(u);
        };
        add_cell(dc);
        nbr8_cells(dc, nb);
        for(int k=0;k<8;++k) add_cell(nb[k]);

        // FALLBACK: If local search found nothing, scan GLOBAL unassigned
        if (cand.empty()) {
        ...
        // Existing cluster: Seed from Boundary
    ...
    for (int ui : cand) {
      ...
      // if (!st.feasible_add(ui, cid)) continue; // Removed feasible check (soft)
      if (st.calc_penalty_add(ui, cid) >= 1e11) continue; // Hard constraint only

      const double d1 = dist_km(st.customer(ui).p, centroid);
      const double d2 = dist_km(st.customer(ui).p, center_pt);

      // bonus if close in cell-space to any boundary member
      ...
      const double score = cfg.w_centroid*d1 + cfg.w_center*d2 + cfg.w_cell*cell_bonus;
